...
void thirdDialog::print(int i, InterStateBuffers &isb, Registry_File &rFile){
   ...
   // if(isb.printISB)
        ...
        if(isb.isMispred){
            ui->countCH->setText(QString::fromStdString(to_string(isb.mispredNumber)));
            ui->CHinst->setText(QString::fromStdString(isb.instructionE));
            ui->BTBpred->setText(QString::fromStdString(to_string(isb.predictionBTB)));
            ui->actualBranch->setText(QString::fromStdString(to_string(!isb.predictionBTB)));
